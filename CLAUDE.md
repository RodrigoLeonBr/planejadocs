...
4. **ocr_engine.py** — scanned via **RapidOCR** (ONNX/CPU) behind the `_run_ocr` seam (lazy import, engine cached in `_engine`). Each page is rasterized (`get_pixmap(dpi=PDF2MD_OCR_DPI).tobytes("png")`) and fed to `RapidOCR()`; text = `"\n\n".join(result.txts)` per page. Any failure → `PDF2MD_004`. `rapidocr` is a core dep (also pulled transitively by pymupdf4llm), so OCR works out of the box — no external binary.
...
- **OCR uses RapidOCR** (ONNX/CPU), chosen over marker-pdf 2.0: for the ~5% scanned docs on a no-GPU host, marker was ~7 min/page and needed an external `llama-server` binary (surya 0.22 dropped its torch backend). RapidOCR is ~18 s/page (incl. one-time engine load; faster per page after), pure-pip, no binary. Trade-off: RapidOCR loses intra-box spacing (joins words) and table structure that marker reconstructed — acceptable for the scanned minority. Validated on a real scanned SMS doc (`demonstrativo_global_aephiva_estadual`). If higher scanned-table fidelity is ever needed, marker/Docling on a GPU box is the escalation.
